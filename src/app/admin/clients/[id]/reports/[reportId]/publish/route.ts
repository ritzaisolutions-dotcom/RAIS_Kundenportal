...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; reportId: string }> }) {
  ...
  if (!mutationSucceeded(updatedRows, error)) {
    return NextResponse.redirect(new URL(`/admin/clients/${id}?tab=reports&error=Veröffentlichen+fehlgeschlagen`, request.url), {
      status: 303,
    });
  }

  return NextResponse.redirect(new URL(`/admin/clients/${id}?tab=reports&success=Report+veröffentlicht`, request.url), { status: 303 });
}
